...
# ============================================================
SRC_VIDEO="${SRC_VIDEO:-input_videos/a_red_rose_bud_in_a_green.mp4}"
...
if ! command -v nvidia-smi >/dev/null 2>&1; then
    echo "ERROR: nvidia-smi not found. Run this inside an GPU session." >&2
    exit 1
fi

nvidia-smi >/dev/null 2>&1 || {
    echo "ERROR: no visible GPU detected. Run this inside an GPU session." >&2
    exit 1
}
...
if type module >/dev/null 2>&1; then
    module load opencv cuda tensorboard || true
fi
...
export WANDB_PROJECT="${WANDB_PROJECT:-sound-sparks-motion}"
