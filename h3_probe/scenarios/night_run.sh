...
ensure_alloc(){ # $1 = seconds needed
  ...
  while :; do log "requesting allocation (2xH100, 3h) ..."
    ...
    if [ -n "$ALLOC" ]; then for i in $(seq 1 60); do alloc_ok && break; sleep 10; done
      if alloc_ok; then log "allocation $ALLOC RUNNING on $(squeue -j $ALLOC -h -o %N), $(secs_left $ALLOC)s"; echo $ALLOC > $ON/alloc_id; return 0; fi; fi
...
run_step(){ # $1 ngpu $2 timeout-seconds $3 logfile $4 command   (sequential; NO --overlap: with it every step sees one GPU)
  ...
  timeout $to srun --jobid=$ALLOC --ntasks=1 --cpus-per-task=8 --gres=gpu:h100:$ng --export=ALL bash -c "cd /home/amirrz/my_codes/Sound_Sparks_Motion; export $BASEENV; $cmd" > $lg 2>&1; local rc=$?
...
while :; do
  ...
  case "$T" in
    ...
    GEN) ensure_alloc 1200; sl=$(echo "$A1" | sed 's/.*//'); run_step 1 1500 $ON/logs/gen_$(date +%H%M%S).log "export PROMPTS_JSON=$A1 ONLY=$(echo $A2 | tr ';' ',') OUT_DIR=/scratch/amirrz/H3_exp/inputs/gen GEN_H=320 GEN_W=512 GEN_FRAMES=124 GEN_SEED=${GEN_SEED:-7}; $PY h3_probe/t2va_sounds.py" || status=fail
    ...
    SCREEN) n=$(echo "$A2" | tr ';' '\n' | grep -c .); ensure_alloc $((600+600*n)); run_step 1 $((900+600*n)) $ON/logs/screen_$(date +%H%M%S).log "export CANDS=$A1 ONLY='$A2' OUT_DIR=$OUTB/screen; $PY h3_probe/scenarios/screen.py" || status=fail
